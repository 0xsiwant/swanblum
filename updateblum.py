        ...
        try:
            ...
            while True:                
                ...
                for index, init_data in enumerate(init_data_lines, start=1):
                    ...
                    TANGGUL = auth(init_data)
                    time.sleep(1)
                    if TANGGUL is None:
                        continue
                    token = TANGGUL['token']['refresh']
                    ...
                    if balance_info is not None:
                        ...
                        if cek_task_enable == 'Y':
                            ...
                            TANGGUL = check_task(token)
                            if TANGGUL is not None:
                                for section in TANGGUL:
                                    ...
                                    for subSection in subSections:
                                        ...
                                        for tasks in subSection['tasks']:
                                            ...
                                            if status == 'READY_FOR_VERIFY':
                                                ...
                                                TANGGUL = validate(token, id_task)
                                                time.sleep(1)
                                                # print(TANGGUL)
                                                if TANGGUL is not None:
                                                    valid_status = TANGGUL.get('status')
                                                    valid_message = TANGGUL.get('message')
                                                    ...
                                                    TANGGUL = claim_task(token, id_task)
                                                    time.sleep(1)
                                                    if TANGGUL is not None:
                                                        status = TANGGUL.get('status')
                                                        message = TANGGUL.get('message')
                                                        if status == 'FINISHED':
                                            ...
                                            if task_type in ['PROGRESS_TARGET', 'OTHER_TYPES_THAT_DO_NOT_SUPPORT_START']:
                                                ...
                                                TANGGUL = start_task(token, id_task)
                                                time.sleep(1)
                                                if TANGGUL is not None: 
